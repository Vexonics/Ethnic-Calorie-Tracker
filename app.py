...
from src.app_logging import log_meal, get_progress
from src.food import search_food, add_food
...
if user is None:
    ...
    if st.button("Log Meal"):
        ...
        if result is None:
            ...
            st.session_state["show_add_food"] = True
        else:
            log_meal(food_name.lower(), servings, meal_type)
            st.success(f"{food_name.title()} logged successfully!")
            st.rerun()

    if st.session_state.get("show_add_food"):
        st.subheader("Add a Custom Food")
        with st.form("add_food_form"):
            new_food_name = st.text_input("Food Name", value=food_name)
            new_serving_size = st.number_input("Serving Size (g)", min_value=1, value=100)
            new_cuisine = st.text_input("Cuisine")
            new_protein = st.number_input("Protein (g)", min_value=0.0)
            new_fiber = st.number_input("Fiber (g)", min_value=0.0)
            new_calories = st.number_input("Calories", min_value=0.0)
            new_carbs = st.number_input("Carbs (g)", min_value=0.0)
            new_sugar = st.number_input("Sugar (g)", min_value=0.0)
            new_sodium = st.number_input("Sodium (mg)", min_value=0.0)
            new_fat = st.number_input("Fat (g)", min_value=0.0)
            ...
            if submitted:
                ...
                st.success(f"{new_food_name.title()} added! You can now log it.")
                st.rerun()
